...
from blink.crossencoder.train_cross import modify, evaluate_edges#, evaluate
...
def _run_crossencoder(crossencoder, dataloader, logger, context_len, device="cuda"):
    ...
    res = evaluate_edges(crossencoder, dataloader, device, logger, context_len, zeshel=False, silent=False) # here it uses the device - cuda or gpu
    ...
    return accuracy, predictions, logits, num_tp_all, nb_eval_examples
...
def _save_predictions(args, samples, predictions, scores, local_id2wikipedia_id, title2id, logger=None):
    ...
    for ind_sample, sample in enumerate(samples):
        ...
        for ind, tit in enumerate(topk_tits):
            tit = '-'.join(tit) # form edge from a tuple of parent and child     
            # highlight the gold concept with '*'s 
            if topk_concepts[ind] == gold_concept:
                the_concept = '*' + '-'.join(topk_concepts[ind]) + '*'
            else:
                the_concept = '-'.join(topk_concepts[ind])
...
def run(
    ...
    while not stopping_condition:
        ...
        context_input, candidate_input, label_input, nns_filtered, tensor_is_NIL_labels = prepare_crossencoder_for_insertion_data(
        ...
        accuracy, index_array, unsorted_scores, num_tp_all, nb_eval_examples = _run_crossencoder(
            crossencoder,
            dataloader,
            logger=logger,
