# get attribute catelogue of SNOMED_CT from .owl file - in BLINK format
...
from onto_snomed_owl_util import load_SNOMEDCT_deeponto,load_SNOMEDCT_owl2dict_ids,deeponto2dict_ids,deeponto2dict_ids_obj_prop,deeponto2dict_ids_ann_prop,get_definition_in_onto_from_iri,get_rdfslabel_in_onto_from_iri,get_preflabel_in_onto_from_iri,get_altlabel_in_onto_from_iri,get_title_in_onto_from_iri,get_entity_info,get_SCTID_from_OWLobj,get_iri_from_SCTID_id,get_SCTID_id_from_iri,get_in_KB_direct_children,get_in_KB_direct_parents,get_entity_graph_info,get_concept_tit_from_id_strs
...
def form_str_ent_row_BLINK(CUI_def,CUI,default_name,list_synonyms,add_synonyms=True,synonym_concat_w_title=False,synonym_as_entity=True,add_direct_hyps=True):
    dict_entity_row = {}
...
parser = argparse.ArgumentParser(description="get the list file of SNOMED-CT attributes")
...
dict_SCTID_onto_older_obj_prop = deeponto2dict_ids_obj_prop(onto_sno)
#dict_SCTID_onto_older_ann_prop = deeponto2dict_ids_ann_prop(onto_sno)

list_entity_json_str = []
for iri in dict_SCTID_onto_older_obj_prop.keys():
    # entity information for in-KB use older onto
    concept_tit, concept_def, concept_syns = get_entity_info(onto_sno,iri)
    #children_str, parents_str, pc_paths_str = get_entity_graph_info(onto_sno,iri,dict_SCTID_onto)
    ...
    if output_format == 'BLINK':
        entity_row_str = form_str_ent_row_BLINK(concept_def,SCTID,concept_tit,list_synonyms,add_synonyms=add_synonyms,synonym_concat_w_title=synonym_concat_w_title,
...
output_to_file('%s/SNOMEDCT-US-%s%s%s%s%s-attributes.jsonl' % (output_data_folder_path, onto_ver, '_syn' if add_synonyms else '', output_syn_mark, '_hyp' if add_direct_hyps else '', ('_' + output_format) if output_format != 'BLINK' else ''),entity_json_str)
...
#     output_to_file('%s/SNOMEDCT-US-%s_with_NIL%s%s%s%s.jsonl' % (output_data_folder_path, onto_ver, '_syn' if add_synonyms else '', output_syn_mark, '_hyp' if add_direct_hyps else '', ('_' + output_format) if output_format != 'BLINK' else ''),entity_json_str)
